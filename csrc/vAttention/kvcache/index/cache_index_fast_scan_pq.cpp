...
void CacheIndexFastScanPQ::bind_fp32(int seqs, int heads, const void *keys) 
{
    ...
    for (int i = 0; i < heads; ++i) { 
        ...
        subsample_training_set(seqs * heads, vector_dim, (const float *)keys, 512, &samples); 
    ...
    for (int i = 0; i < heads; ++i) {
        pq[0]->compute_codes(
            seqs, 
            (const float *)keys + (size_t)i * vector_dim, 
            codes.data() + i * align_seqs * pq[0]->code_size);
    }
